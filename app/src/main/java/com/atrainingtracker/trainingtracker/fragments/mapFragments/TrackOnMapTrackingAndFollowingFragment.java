...
public class TrackOnMapTrackingAndFollowingFragment extends TrackOnMapTrackingFragment {
    ...
    private class FollowMeLocationSource implements LocationSource, LocationListener {
        ...
        CameraPosition getCameraPosition(Location location) {
            ...
            return mCameraPositionBuilder.target(new LatLng(location.getLatitude(), location.getLongitude())).bearing(location.getBearing()).tilt(90)  //  always the highest possible tilt // mMap.getCameraPosition().tilt)
                    .zoom(speed2zoom(location.getSpeed(), mMap.getCameraPosition().zoom)).build();                   // Creates a CameraPosition from the builder
        }
